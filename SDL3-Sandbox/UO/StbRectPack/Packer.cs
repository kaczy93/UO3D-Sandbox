...
namespace StbRectPackSharp
{
    ...
    public unsafe class Packer : IDisposable
	{
		...
		/// <returns></returns>
		public bool PackRect(int width, int height, out SDL_Rect packRectangle, int offset = 2)
		{
			...
			packRectangle = new SDL_Rect
			{
                x = rect.x + (int) (offset / 2f),
                y = rect.y + (int) (offset / 2f),
                w = rect.w - offset,
                h = rect.h - offset
            };
